"""Persistence tests for RAES participant behavior-history and evidence records (#1289)."""
...
from shared.schemas.raes_participant_runtime import RaesParticipantRuntimeRecordError
...
def test_persist_evidence_record_is_idempotent():
    ...
    assert first.record_kind == RaesParticipantRuntimeRecord.RecordKind.PARTICIPANT_EVIDENCE
    assert first.contract_version == "participant-evidence-v1"
    assert first.owner == RaesParticipantRuntimeRecord.Owner.SHARED
...
def test_persist_evidence_record_owner_override():
    row = _evidence(options=ParticipantRecordWriteOptions(owner=RaesParticipantRuntimeRecord.Owner.CTF))
    assert row.owner == RaesParticipantRuntimeRecord.Owner.CTF
...
def test_persist_evidence_rejects_participant_ref_payload_mismatch():
    ...
    with pytest.raises(RaesParticipantRuntimeRecordConflict, match="conflicts with identity"):
        _evidence(
...
def test_persist_behavior_history_record_is_idempotent():
    ...
    assert first.id == second.id
    assert first.record_kind == RaesParticipantRuntimeRecord.RecordKind.PARTICIPANT_BEHAVIOR_HISTORY
