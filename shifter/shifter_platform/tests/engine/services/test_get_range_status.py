...
class TestGetRangeStatus:
    ...
    def test_handles_null_fields(self, user):
        range_obj = Range.objects.create(
            workspace_id=_WORKSPACE_ID, user=user, status=Range.Status.PENDING, provisioned_instances=None
        )
        result = get_range_status(range_obj.id)
    ...
    def test_does_not_modify_range(self, user):
        range_obj = Range.objects.create(
            workspace_id=_WORKSPACE_ID, user=user, status=Range.Status.READY, error_message="orig"
        )
