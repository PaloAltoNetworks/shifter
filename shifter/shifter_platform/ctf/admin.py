...
class CTFChallengeAdmin(SoftDeleteAdminMixin, admin.ModelAdmin):
    ...
    def get_queryset(self, request: HttpRequest) -> QuerySet:
        ...
        qs = super().get_queryset(request)
        return qs.annotate(
            _solve_count=Count("submissions", filter=admin.models.Q(submissions__is_correct=True))
        )
        ...
        return qs.annotate(
            ...
            _total_score=Coalesce(
                Sum(
                    ...
                    filter=admin.models.Q(members__submissions__is_correct=True),
                ),
                0,
            ),
        )
        ...
        return qs.annotate(
            _total_score=Coalesce(
                Sum(
                    ...
                    filter=admin.models.Q(submissions__is_correct=True),
                ),
                0,
            ),
            _solved_count=Count(
