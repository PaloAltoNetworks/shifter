...
export class SSHConnectionManager {
  ...
  private async getConnection(
    ...
    const connectionKey = `minetest-server-${username}@${host}:${port}`;
    console.error(`[SSH-SERVER] getConnection called with key: ${connectionKey}`);
    
    if (this.connections.has(connectionKey)) {
      const connInfo = this.connections.get(connectionKey)!;
      if (connInfo.connected) {
        console.error(`[SSH-SERVER] Reusing existing connection for: ${connectionKey}`);
        return connInfo.client;
      }
      // Connection is dead, remove it
      console.error(`[SSH-SERVER] Removing dead connection for: ${connectionKey}`);
      this.connections.delete(connectionKey);
    }

    // Create new connection
    console.error(`[SSH-SERVER] Creating new connection for: ${connectionKey}`);
    ...
    console.error(`[SSH-SERVER] Connection cache now has ${this.connections.size} connections`);
