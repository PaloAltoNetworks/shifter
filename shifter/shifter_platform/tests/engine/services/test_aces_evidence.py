"""Tests for the ACES provisioner-evidence consume path (#1478).

The provisioner emits range.aces.operation / range.aces.snapshot outbox events;
...
def _operation_event(status: str = "running", **overrides) -> dict:
    event = {
        "event_type": "range.aces.operation",
        "event_id": "evt-op-1",
...
def _snapshot_event(**overrides) -> dict:
    event = {
        "event_type": "range.aces.snapshot",
        "event_id": "evt-snap-1",
        "request_id": REQUEST_ID,
        "range_id": 7,
        "user_id": 3,
...
def test_operation_event_persists_operation_status_record():
    record_aces_operation_status(_operation_event(status="succeeded", status_reason="done"))
    rec = AcesOperationRecord.objects.get(record_kind=AcesOperationRecord.RecordKind.OPERATION_STATUS)
...
def test_snapshot_event_persists_runtime_snapshot_record():
    record_aces_runtime_snapshot(_snapshot_event())
    rec = AcesOperationRecord.objects.get(record_kind=AcesOperationRecord.RecordKind.RUNTIME_SNAPSHOT)
...
def test_malformed_operation_event_is_ignored_not_fatal():
    record_aces_operation_status({"event_type": "range.aces.operation", "event_id": "e"})
    assert not AcesOperationRecord.objects.filter(record_kind=AcesOperationRecord.RecordKind.OPERATION_STATUS).exists()


@pytest.mark.django_db
def test_process_range_event_routes_aces_events_to_records():
    ...
    kinds = set(AcesOperationRecord.objects.values_list("record_kind", flat=True))
    assert {
