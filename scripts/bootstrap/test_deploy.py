    ...
    def smart_subprocess(cmd, **kwargs):
        ...
        if "which" in cmd_str and "gh" in cmd_str:
            return subprocess.CompletedProcess(
                args=cmd, returncode=0, stdout="/usr/bin/gh\n", stderr=""
            )
        ...
        # Terraform output -json
            ...
            return subprocess.CompletedProcess(
                args=cmd, returncode=0,
                stdout='{"test_output": {"value": "test"}}',
                stderr=""
            )
        # Terraform show (for displaying plan)
        elif "terraform" in cmd_str and "show" in cmd_str:
            return subprocess.CompletedProcess(
                args=cmd, returncode=0, stdout="terraform plan output...", stderr=""
            )
        # Git commands (add, commit, push)
        elif "git" in cmd_str and any(x in cmd_str for x in ["add", "commit", "push"]):
            return subprocess.CompletedProcess(
                args=cmd, returncode=0, stdout="", stderr=""
            )
        # gh secret set
        elif "gh" in cmd_str and "secret" in cmd_str and "set" in cmd_str:
            return subprocess.CompletedProcess(
                args=cmd, returncode=0, stdout="", stderr=""
            )
        # Default success
...
class TestCheckDependencies:
    ...
    def test_succeeds_when_all_required_dependencies_present(self):
        ...
        with patch("shutil.which") as mock_which:
            mock_which.side_effect = lambda cmd: (
                "/usr/bin/aws" if cmd == "aws"
                else "/usr/bin/terraform" if cmd == "terraform"
                else "/usr/bin/git" if cmd == "git"
                else "/usr/bin/gh" if cmd == "gh"
                else None
            )
    ...
    def test_exits_when_terraform_missing(self):
        ...
        with patch("shutil.which") as mock_which:
            mock_which.side_effect = lambda cmd: (
                "/usr/bin/aws" if cmd == "aws"
                else None if cmd == "terraform"  # terraform missing
                else "/usr/bin/git" if cmd == "git"
                else None
            )
    ...
    def test_exits_when_git_missing(self):
        ...
        with patch("shutil.which") as mock_which:
            mock_which.side_effect = lambda cmd: (
                ...
                else None if cmd == "git"  # git missing
                else None
            )
...
class TestGetAwsAccountId:
    ...
    def test_returns_account_id_from_aws_cli(self):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = subprocess.CompletedProcess(
                args=[], returncode=0, stdout="123456789012\n"
            )
...
class TestBootstrapAccount:
    ...
    def test_creates_s3_bucket_for_terraform_state(
        self, bootstrap_config, mock_subprocess
    ):
        ...
        with (
            ...
            s3_calls = [c for c in mock_subprocess.call_args_list
                       if len(c[0]) > 0 and len(c[0][0]) > 0 and
                       c[0][0][0] == "aws" and "s3" in " ".join(c[0][0])]
            assert len(s3_calls) > 0

    def test_creates_dynamodb_table_for_state_locking(
        self, bootstrap_config, mock_subprocess
    ):
        ...
        with (
            ...
            dynamo_calls = [c for c in mock_subprocess.call_args_list
                           if len(c[0]) > 0 and len(c[0][0]) > 0 and
                           c[0][0][0] == "aws" and "dynamodb" in " ".join(c[0][0])]
            assert len(dynamo_calls) > 0

    def test_creates_github_oidc_provider(
        self, bootstrap_config, mock_subprocess
    ):
        ...
        with (
            ...
            oidc_calls = [c for c in mock_subprocess.call_args_list
                         if len(c[0]) > 0 and len(c[0][0]) > 0 and
                         c[0][0][0] == "aws" and
                            "create-open-id-connect-provider" in " ".join(c[0][0])]
            assert len(oidc_calls) > 0

    def test_creates_iam_role_for_github_actions(
        self, bootstrap_config, mock_subprocess
    ):
        ...
        with (
            ...
            role_calls = [c for c in mock_subprocess.call_args_list
                         if len(c[0]) > 0 and len(c[0][0]) > 0 and
                         c[0][0][0] == "aws" and "create-role" in " ".join(c[0][0])]
            assert len(role_calls) > 0

    def test_attaches_admin_policy_to_role(
        self, bootstrap_config, mock_subprocess
    ):
        ...
        with (
            ...
            policy_calls = [c for c in mock_subprocess.call_args_list
                           if len(c[0]) > 0 and len(c[0][0]) > 0 and
                           c[0][0][0] == "aws" and
                              "put-role-policy" in " ".join(c[0][0])]
            assert len(policy_calls) > 0

    def test_returns_dict_with_bootstrap_results(
        self, bootstrap_config, mock_subprocess
    ):
    ...
    def test_uses_correct_github_org_and_repo_in_trust_policy(
        self, bootstrap_config, mock_subprocess
    ):
        ...
        with (
            ...
            role_calls = [c for c in mock_subprocess.call_args_list
                         if len(c[0]) > 0 and len(c[0][0]) > 1 and
                         "create-role" in " ".join(c[0][0])]
            assert len(role_calls) > 0
    ...
    def test_does_not_create_resources_in_dry_run(self, bootstrap_config):
        ...
        with (
            ...
            result = deploy.bootstrap_account(
                bootstrap_config, "my-profile", dry_run=True
            )
    ...
    def test_passes_profile_to_all_aws_commands(self, bootstrap_config):
        ...
        with (
            ...
            aws_calls = [c for c in mock_run.call_args_list
                        if len(c[0]) > 0 and len(c[0][0]) > 0 and c[0][0][0] == "aws"]
...
class TestWalkthroughGithubSecrets:
    ...
    def test_sets_secret_via_gh_cli_when_user_confirms(
        self, bootstrap_config, mock_stdin_tty
    ):
    ...
    def test_includes_role_arn_in_gh_secret_command(
        self, bootstrap_config, mock_stdin_tty
    ):
        ...
        with (
            ...
            gh_calls = [c for c in mock_run.call_args_list
                       if c[0][0][0] == "gh" and "set" in c[0][0]]
            assert len(gh_calls) > 0
    ...
    def test_provides_manual_instructions_when_gh_not_available(
        self, capsys, mock_stdin_tty
    ):
...
class TestWalkthroughBackendConfig:
    ...
    def test_writes_backend_tf_files_when_user_confirms(
        self, mock_repo_root, mock_stdin_tty, mock_subprocess
    ):
    ...
    def test_creates_correct_backend_config_content(
        self, mock_repo_root, mock_stdin_tty, mock_subprocess
    ):
    ...
    def test_commits_and_pushes_when_user_confirms(
        self, mock_repo_root, mock_stdin_tty, mock_subprocess
    ):
        ...
        with (
            ...
            git_calls = [c for c in mock_subprocess.call_args_list
                        if len(c[0]) > 0 and len(c[0][0]) > 0 and c[0][0][0] == "git"]
    ...
    def test_exits_when_user_refuses_backend_config(
        self, mock_repo_root, mock_stdin_tty, mock_subprocess
    ):
    ...
    def test_exits_when_file_write_fails(
        self, mock_repo_root, mock_stdin_tty, mock_subprocess
    ):
    ...
    def test_exits_when_git_commit_fails_after_confirmation(
        self, mock_repo_root, mock_stdin_tty
    ):
...
class TestTerraformDeploy:
    ...
    def test_runs_terraform_init_for_all_components(
        self, mock_repo_root, mock_stdin_tty, mock_subprocess
    ):
        ...
        with (
            ...
            init_calls = [c for c in mock_subprocess.call_args_list
                         if len(c[0]) > 0 and len(c[0][0]) > 0 and
                         "terraform" in " ".join(c[0][0]) and "init" in " ".join(c[0][0])]
            assert len(init_calls) == 3

    def test_runs_terraform_plan_for_all_components(
        self, mock_repo_root, mock_stdin_tty, mock_subprocess
    ):
        ...
        with (
            ...
            plan_calls = [c for c in mock_subprocess.call_args_list
                         if len(c[0]) > 0 and len(c[0][0]) > 0 and
                         c[0][0][0] == "terraform" and len(c[0][0]) > 1 and c[0][0][1] == "plan"]
            assert len(plan_calls) == 3

    def test_runs_terraform_apply_when_user_confirms(
        self, mock_repo_root, mock_stdin_tty, mock_subprocess
    ):
        ...
        with (
            ...
            apply_calls = [c for c in mock_subprocess.call_args_list
                          if len(c[0]) > 0 and len(c[0][0]) > 0 and
                          "terraform" in " ".join(c[0][0]) and "apply" in " ".join(c[0][0])]
            assert len(apply_calls) == 3

    def test_captures_terraform_outputs(
        self, mock_repo_root, mock_stdin_tty, mock_subprocess
    ):
    ...
    def test_exits_when_terraform_init_fails(
        self, mock_repo_root, mock_stdin_tty
    ):
    ...
    def test_exits_when_terraform_plan_fails(
        self, mock_repo_root, mock_stdin_tty
    ):
    ...
    def test_exits_when_user_refuses_terraform_apply(
        self, mock_repo_root, mock_stdin_tty
    ):
    ...
    def test_exits_when_terraform_apply_fails(
        self, mock_repo_root, mock_stdin_tty
    ):
    ...
    def test_does_not_execute_terraform_in_dry_run(
        self, mock_repo_root, mock_subprocess
    ):
    ...
    def test_deploys_components_in_correct_order(
        self, mock_repo_root, mock_stdin_tty, mock_subprocess
    ):
...
class TestWalkthroughCognitoUser:
    ...
    def test_creates_cognito_user_with_email(self, mock_stdin_tty, mock_subprocess):
        ...
        outputs = {
            "cognito_user_pool_id": {"value": "us-east-2_ABC123"}
        }
        ...
        with (
            ...
            cognito_calls = [c for c in mock_subprocess.call_args_list
                            if len(c[0]) > 0 and len(c[0][0]) > 0 and
                            c[0][0][0] == "aws" and "cognito-idp" in " ".join(c[0][0])]
    ...
    def test_includes_user_pool_id_in_command(self, mock_stdin_tty, mock_subprocess):
        ...
        outputs = {
            "cognito_user_pool_id": {"value": "us-east-2_TESTPOOL"}
        }
        ...
        with (
            ...
            cognito_calls = [c for c in mock_subprocess.call_args_list
                            if len(c[0]) > 0 and len(c[0][0]) > 0 and
                            c[0][0][0] == "aws" and "admin-create-user" in " ".join(c[0][0])]
            assert len(cognito_calls) > 0
    ...
    def test_does_not_create_user_in_dry_run_mode(self, mock_subprocess):
        ...
        with patch("deploy.run_cmd") as mock_run:
            deploy.walkthrough_cognito_user(
                outputs, "dev", "my-profile", dry_run=True
            )
