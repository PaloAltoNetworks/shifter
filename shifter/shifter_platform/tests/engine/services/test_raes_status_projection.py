"""Behavior tests for RAES operation-status projection (engine.services, #1274)."""
...
from engine.services import project_raes_operation_status
from shared.enums import ResourceStatus
from shared.messages.events import EVENT_TYPE_STATUS_UPDATED
from shared.models import RaesOperationRecord
from shared.raes.status import ProjectionDecision, RaesOperationStatusObservation, RangeOperation
...
def user(django_user_model):
    return django_user_model.objects.create_user(username="raes-proj@example.com", email="raes-proj@example.com")
...
def _project(**kwargs):
    ...
    return project_raes_operation_status(RaesOperationStatusObservation(**kwargs))


def _status_records(request_id):
    return RaesOperationRecord.objects.filter(
...
def test_long_status_reason_is_bounded_in_event_and_sidecar(user):
    from shared.raes.status import MAX_DIAGNOSTIC_TEXT_LEN
