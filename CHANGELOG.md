...
- **Pinned GCP dev overlay image tags to commit SHA during deploys.**
  `_gcp-dev.yml` now rewrites `platform/k8s/gcp/overlays/gcp-dev/kustomization.yaml`
  `newTag` entries from `latest` to `${SHORT_SHA}` before `kubectl apply -k`.
  This makes rollouts deterministic for the exact commit being deployed instead
  of whichever image currently sits behind mutable `:latest`.
