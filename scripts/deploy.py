...
Usage:
    ./scripts/deploy.py bootstrap --env prod
    ./scripts/deploy.py bootstrap --env prod --dry-run
    ./scripts/deploy.py terraform --env prod
    ./scripts/deploy.py terraform --env prod --dry-run
...
def run_cmd(cmd: list[str], dry_run: bool = False, check: bool = True, capture: bool = False) -> subprocess.CompletedProcess | None:
...
def bootstrap_account(config: BootstrapConfig, dry_run: bool = False) -> dict:
    ...
    ], dry_run=dry_run, check=False)  # May already exist
...
def walkthrough_github_secrets(bootstrap_result: dict, dry_run: bool = False) -> None:
    ...
    print("  1. Go to: https://github.com/Brad-Edwards/shifter/settings/secrets/actions")
...
def terraform_deploy(env: str, dry_run: bool = False) -> dict:
...
def walkthrough_cognito_user(outputs: dict, env: str, dry_run: bool = False) -> None:
    ...
    if "cognito_user_pool_id" in outputs:
        ...
        email = "admin@paloaltonetworks.com"
...
def full_deployment(env: str, dry_run: bool = False) -> None:
...
""")
    ...
    bootstrap_result = bootstrap_account(config, dry_run=dry_run)
    ...
    if not dry_run:
        if not confirm("Continue with Terraform deployment?"):
            ...
            code_block(f"./scripts/deploy.py terraform --env {env}")
            return
    
    outputs = terraform_deploy(env, dry_run=dry_run)
    ...
    if not dry_run and outputs:
        ...
        # Phase 7: First User
        walkthrough_cognito_user(outputs, env, dry_run=dry_run)
...
Examples:
  ...
  AWS_PROFILE=<profile> ./scripts/deploy.py full --env prod
  
  # Just bootstrap AWS account
  AWS_PROFILE=<profile> ./scripts/deploy.py bootstrap --env prod
  
  # Just run terraform (after bootstrap)
  AWS_PROFILE=<profile> ./scripts/deploy.py terraform --env prod
        """
    )
    ...
    if args.command == "bootstrap":
        ...
        result = bootstrap_account(config, dry_run=args.dry_run)
        if not args.dry_run:
        ...
        outputs = terraform_deploy(args.env, dry_run=args.dry_run)
        if not args.dry_run and outputs:
            ...
            walkthrough_cognito_user(outputs, args.env, dry_run=args.dry_run)
            walkthrough_final_steps(args.env)
    
    elif args.command == "full":
        full_deployment(args.env, dry_run=args.dry_run)
