"""Generic RAES sidecar validation primitives shared across record families.

Extracted from :mod:`shared.schemas.raes_operation` (#1288) so new RAES
...
:class:`RaesRecordError`) so each domain module can raise its own
domain-specific error subclass (for example ``RaesOperationRecordError`` or
``RaesParticipantRuntimeRecordError``) while sharing one validation
implementation.
"""
...
class RaesRecordError(ValueError):
    """Raised when an RAES sidecar record violates a shared storage contract."""


def canonical_raes_payload_digest(payload: object, *, error_cls: type[Exception] = RaesRecordError) -> str:
...
def _reject_secret_key(path: str, key: str, *, error_cls: type[Exception] = RaesRecordError) -> None:
...
def _reject_secret_value(path: str, value: str, *, error_cls: type[Exception] = RaesRecordError) -> None:
...
def _require_single_line_ref(
    ...
    *,
    ...
    error_cls: type[Exception] = RaesRecordError,
) -> str:
...
def _require_digest(name: str, value: object, *, error_cls: type[Exception] = RaesRecordError) -> str:
...
def _require_uuid(
    name: str, value: object, *, required: bool, error_cls: type[Exception] = RaesRecordError
) -> UUID | None:
...
def _require_aware_datetime(name: str, value: object, *, error_cls: type[Exception] = RaesRecordError) -> datetime:
...
def _require_json_size(
    name: str, value: object, *, max_bytes: int, error_cls: type[Exception] = RaesRecordError
) -> None:
...
def _validate_json_value(path: str, value: object, *, error_cls: type[Exception] = RaesRecordError) -> JsonValue:
...
def _validate_json_scalar(path: str, value: object, *, error_cls: type[Exception] = RaesRecordError) -> Scalar:
...
def _validate_json_object(
    path: str, value: dict[object, object], *, error_cls: type[Exception] = RaesRecordError
) -> JsonObject:
...
#: Allowlisted, reference-only diagnostic keys shared by every RAES sidecar
...
def validate_diagnostic_refs(
    ...
    *,
    ...
    single implementation shared by every RAES sidecar family so the
