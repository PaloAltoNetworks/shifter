"""Tests for the runtime-safe RAES operation-status -> ResourceStatus adapter (#1274)."""
...
def test_mapping_covers_every_intent_and_state_combination():
    ...
    assert set(EXPECTED_MAP.keys()) == {(intent, state) for intent in RangeOperation for state in RAES_OPERATION_STATES}
...
def test_projection_is_frozen():
    ...
    assert isinstance(result, RaesStatusProjection)
    with pytest.raises(FrozenInstanceError):
        result.decision = ProjectionDecision.STALE  # type: ignore[misc]


def test_runtime_states_match_raes_enum():
    """Drift guard: the runtime-safe state set must match the RAES SDL enum.

    The SDL is a dev/test dependency only; the runtime adapter never imports it.
    """
    runtime_state = pytest.importorskip("raes_contracts.runtime_state")
    sdl_states = {member.value for member in runtime_state.OperationState}
    assert sdl_states == RAES_OPERATION_STATES
