"""Shared-native validation for RAES participant-runtime sidecar records (#1288).

Mirrors :mod:`shared.schemas.raes_operation` (the incumbent
:class:`shared.models.RaesOperationRecord` validation pattern), reusing the
generic primitives extracted to :mod:`shared.schemas._raes_validation`. This
...
#: Column-aligned caps (see ``shared.models.RaesParticipantRuntimeRecord``): the
...
class RaesParticipantRuntimeRecordError(RaesRecordError):
    """Raised when an RAES participant-runtime sidecar record violates the storage contract."""


@dataclass(frozen=True)
class RaesParticipantRuntimeRecordData:
...
@dataclass(frozen=True)
class ValidatedRaesParticipantRuntimeRecord:
...
def _validate_payload(record_kind: str, payload: object) -> JsonObject:
    ...
    if not isinstance(payload, dict):
        raise RaesParticipantRuntimeRecordError("payload must be a JSON object")
    for key in payload:
        if not isinstance(key, str):
            raise RaesParticipantRuntimeRecordError("payload keys must be strings")
        _reject_secret_key("payload", key, error_cls=RaesParticipantRuntimeRecordError)
    allowed_keys = PAYLOAD_KEYS_BY_RECORD_KIND[record_kind]
    extra_keys = set(payload) - allowed_keys
    if extra_keys:
        raise RaesParticipantRuntimeRecordError(
    ...
    _require_json_size("payload", payload, max_bytes=_MAX_JSON_BYTES, error_cls=RaesParticipantRuntimeRecordError)
    validated = _validate_json_value("payload", payload, error_cls=RaesParticipantRuntimeRecordError)
...
def _require_enum(name: str, value: object, allowed: frozenset[str]) -> None:
    ...
    if value not in allowed:
        raise RaesParticipantRuntimeRecordError(f"{name} must be one of {sorted(allowed)}")
...
def _validate_evidence_fields(payload: JsonObject) -> None:
    ...
    _require_single_line_ref(
        "provenance_ref", payload.get("provenance_ref"), required=True, error_cls=RaesParticipantRuntimeRecordError
    )
    for field in _EVIDENCE_OPTIONAL_REF_FIELDS:
        if field in payload:
            _require_single_line_ref(field, payload[field], required=False, error_cls=RaesParticipantRuntimeRecordError)
    if "artifact_digest" in payload:
        _require_digest("artifact_digest", payload["artifact_digest"], error_cls=RaesParticipantRuntimeRecordError)
...
def _validate_behavior_history_fields(payload: JsonObject) -> None:
    ...
    _require_single_line_ref(
        "event_ref", payload.get("event_ref"), required=True, error_cls=RaesParticipantRuntimeRecordError
    )
    if "operation_id" in payload:
        _require_single_line_ref(
            "operation_id", payload["operation_id"], required=False, error_cls=RaesParticipantRuntimeRecordError
        )
    if "event_digest" in payload:
        _require_digest("event_digest", payload["event_digest"], error_cls=RaesParticipantRuntimeRecordError)
    if "sequence" in payload:
        sequence = payload["sequence"]
        if not isinstance(sequence, int) or isinstance(sequence, bool) or sequence < 0:
            raise RaesParticipantRuntimeRecordError("sequence must be a non-negative integer")
...
def validate_raes_participant_runtime_record(
    record: RaesParticipantRuntimeRecordData,
) -> ValidatedRaesParticipantRuntimeRecord:
    """Validate an RAES participant-runtime sidecar record before persistence."""

    _require_uuid("request_id", record.request_id, required=True, error_cls=RaesParticipantRuntimeRecordError)
    _require_uuid("range_id", record.range_id, required=False, error_cls=RaesParticipantRuntimeRecordError)
    _require_uuid(
        "range_instance_id", record.range_instance_id, required=False, error_cls=RaesParticipantRuntimeRecordError
    )
    _require_single_line_ref(
        "participant_ref",
        ...
        error_cls=RaesParticipantRuntimeRecordError,
    )
    _require_single_line_ref(
        "idempotency_key",
    ...
    if record.contract_kind != "raes":
        raise RaesParticipantRuntimeRecordError("contract_kind must be 'raes'")
    if record.contract_profile != SHIFTER_BACKEND_PROFILE:
        raise RaesParticipantRuntimeRecordError(f"contract_profile must be {SHIFTER_BACKEND_PROFILE!r}")
    ...
    if record.owner not in OWNER_VALUES:
        raise RaesParticipantRuntimeRecordError(f"owner must be one of {sorted(OWNER_VALUES)}")
    if record.retention_class not in RETENTION_CLASS_VALUES:
        raise RaesParticipantRuntimeRecordError(f"retention_class must be one of {sorted(RETENTION_CLASS_VALUES)}")
    if record.redaction_state not in REDACTION_STATE_VALUES:
        raise RaesParticipantRuntimeRecordError(f"redaction_state must be one of {sorted(REDACTION_STATE_VALUES)}")
    _require_aware_datetime("source_timestamp", record.source_timestamp, error_cls=RaesParticipantRuntimeRecordError)
    _require_digest("payload_digest", record.payload_digest, error_cls=RaesParticipantRuntimeRecordError)
    payload = _validate_payload(record.record_kind, record.payload)
    expected_digest = canonical_raes_payload_digest(payload, error_cls=RaesParticipantRuntimeRecordError)
    if record.payload_digest != expected_digest:
        raise RaesParticipantRuntimeRecordError("payload_digest must match canonical payload digest")
    return ValidatedRaesParticipantRuntimeRecord(
        payload=payload,
        diagnostic_refs=validate_diagnostic_refs(record.diagnostic_refs, error_cls=RaesParticipantRuntimeRecordError),
    )
