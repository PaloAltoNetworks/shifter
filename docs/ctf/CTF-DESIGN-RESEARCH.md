...
2. **Layered hints**: Progressive hint system where each hint reveals more detail. PicoCTF
   uses embedded hints that do not penalize score. Consider a point-cost hint system (for example
...
**Parameters**:
- `initial`: Starting point value (for example 500)
- `minimum`: Floor point value (for example 100)
- `decay`: Number of solves before reaching minimum (for example 20)
