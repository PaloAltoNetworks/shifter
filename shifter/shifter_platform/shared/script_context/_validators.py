...
from shared.template_vars import ALLOWED_PROPERTIES
...
def _validate_instance_id(v: str) -> str:
    ...
    if not _INSTANCE_ID_PATTERN.fullmatch(v):
        raise ValueError("instance_id must match 'i-' followed by 8..17 lowercase hex characters")
    return v
...
def _validate_prompt_text(v: str) -> str:
    ...
    if len(v) > _MAX_PROMPT_TEXT:
        raise ValueError(f"claude_prompt_resolved exceeds {_MAX_PROMPT_TEXT} characters")
...
# (defined in `shared.template_vars` and the source of truth for
...
if _missing_property_validators:
    raise ImportError(
        "shared.script_context: missing typed validators for template "
        f"properties {_missing_property_validators}. Update _PROPERTY_VALIDATORS "
        "in lockstep with shared.template_vars.ALLOWED_PROPERTIES."
    )
...
def _validate_template_substitutions(
    ...
    `shared.template_vars.build_instance_data` from raw
