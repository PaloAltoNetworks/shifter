"""Read-only projections of RAES operation sidecar records for product APIs (#1275).

The sidecar (:class:`shared.models.RaesOperationRecord`) is written through
:mod:`shared.raes.operations` after :mod:`shared.schemas.raes_operation`
...
return" (preflight ``docs/adr/index.yaml``)
...
# ``shared.schemas.raes_operation.PAYLOAD_KEYS_BY_RECORD_KIND``; nested raw
...
RESPONSE_PAYLOAD_KEYS_BY_RECORD_KIND: dict[str, frozenset[str]] = {
    RaesOperationRecord.RecordKind.OPERATION_RECEIPT: frozenset(
        {"operation_id", "status", "accepted", "source_timestamp", "receipt_digest", "receipt_ref"}
    ),
    RaesOperationRecord.RecordKind.OPERATION_STATUS: frozenset(
        {"operation_id", "status", "status_reason", "source_timestamp", "updated_at"}
    ),
    RaesOperationRecord.RecordKind.RUNTIME_SNAPSHOT: frozenset(
...
RECORD_KIND_OPERATION_RECEIPT: str = RaesOperationRecord.RecordKind.OPERATION_RECEIPT
RECORD_KIND_OPERATION_STATUS: str = RaesOperationRecord.RecordKind.OPERATION_STATUS
RECORD_KIND_RUNTIME_SNAPSHOT: str = RaesOperationRecord.RecordKind.RUNTIME_SNAPSHOT
...
class RaesOperationRecordProjection:
    """Serializer-ready, redacted view of one :class:`RaesOperationRecord`.
...
def _redacted_payload(record: RaesOperationRecord) -> dict[str, Any]:
...
def _project(record: RaesOperationRecord) -> RaesOperationRecordProjection:
    ...
    return RaesOperationRecordProjection(
        id=record.id,
...
def list_operation_records(
    ...
    *,
...
) -> list[RaesOperationRecordProjection]:
    ...
    ``RaesOperationRecord`` model. The per-kind response allowlist is applied
    ...
    rows = RaesOperationRecord.objects.filter(
