"""Compact presentation projection of RAES operation state for range UI surfaces (#1276).

Mission Control range cards want a *secondary, read-only* view of RAES operation
state alongside the authoritative Shifter range lifecycle. This module is the
single presentation seam over the #1275 read API (:mod:`shared.raes.projections`):
...
- it reads through ``shared.raes.projections.list_operation_records`` (already
  response-allowlisted) and never touches ``RaesOperationRecord`` or raw payloads;
- runtime snapshots are reduced to a resource *count* plus a stable reference --
  ...
  (preflight ``docs/adr/index.yaml``);
- RAES operation status is mapped to a display label kept explicitly distinct
  from :class:`shared.enums.ResourceStatus`, so the UI never conflates the RAES
  operation lifecycle with the Shifter range lifecycle;
- it returns ``None`` for legacy / non-RAES ranges (no sidecar rows), so those
...
from shared.raes.participant_runtime_projections import (
    ...
    RaesParticipantRuntimeRecordProjection,
...
from shared.raes.status import (
    RAES_STATE_ACCEPTED,
    RAES_STATE_CANCELLED,
    RAES_STATE_FAILED,
    RAES_STATE_RUNNING,
    RAES_STATE_SUCCEEDED,
)
...
#: Display labels for the RAES operation lifecycle. Phrased with an "Operation"
#: prefix so they never collide with Shifter ``ResourceStatus`` display values;
#: a drift test asserts distinctness.
RAES_OPERATION_STATUS_LABELS: dict[str, str] = {
    RAES_STATE_ACCEPTED: "Operation accepted",
    RAES_STATE_RUNNING: "Operation running",
    RAES_STATE_SUCCEEDED: "Operation succeeded",
    RAES_STATE_FAILED: "Operation failed",
    RAES_STATE_CANCELLED: "Operation cancelled",
}
...
def status_label_for(status: str | None) -> str:
    """Return the UI display label for an RAES operation status."""
    if status is None:
        return UNKNOWN_STATUS_LABEL
    return RAES_OPERATION_STATUS_LABELS.get(status, UNKNOWN_STATUS_LABEL)
...
class RangeRaesSnapshotSummary:
...
@dataclass(frozen=True)
class RangeRaesProjection:
    """Compact, UI-safe summary of a range's latest RAES operation state.

    ``status`` is the raw RAES operation state; ``status_label`` is the
    ...
    snapshot: RangeRaesSnapshotSummary | None
    receipt: RangeRaesReceiptSummary | None
...
def _latest(request_id: UUID | str, record_kind: str, contract_profile: str) -> RaesOperationRecordProjection | None:
...
def _snapshot_summary(record: RaesOperationRecordProjection) -> RangeRaesSnapshotSummary:
    ...
    return RangeRaesSnapshotSummary(
...
def build_range_raes_projection(
    request_id: UUID | str,
    *,
    contract_profile: str = SHIFTER_BACKEND_PROFILE,
) -> RangeRaesProjection | None:
    """Build the compact RAES projection for a range's ``request_id``.
    ...
    Returns ``None`` when the range has no RAES sidecar rows (legacy / non-RAES
...
class RangeParticipantRuntimeProjection:
    """Compact, UI-safe summary of a range's RAES participant/runtime state (#1290).

    Sibling to :class:`RangeRaesProjection`: ``participants`` is a bounded,
    ...
    rows, so non-RAES ranges are unaffected.
    """
...
def _implementation_summary(record: RaesParticipantRuntimeRecordProjection) -> dict[str, Any]:
...
def _runtime_summary(record: RaesParticipantRuntimeRecordProjection) -> dict[str, Any]:
...
def _latest_by_participant(
    records: list[RaesParticipantRuntimeRecordProjection],
) -> dict[str, RaesParticipantRuntimeRecordProjection]:
    """Reduce newest-first records to the latest row per ``participant_ref``."""
    latest: dict[str, RaesParticipantRuntimeRecordProjection] = {}
...
def build_range_participant_runtime_projection(
    ...
    *,
    ...
    the range has zero participant rows of either kind (legacy / non-RAES
    ranges), so callers can treat the projection as absent -- mirrors
    :func:`build_range_raes_projection`.
    """
