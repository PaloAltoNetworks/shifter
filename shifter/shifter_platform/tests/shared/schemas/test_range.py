...
class TestRangeRef:
    ...
    # ---------------------------------------------------------------------
    # from_instance class method
    # ---------------------------------------------------------------------

    def test_from_instance_creates_range_ref(self):
        """from_instance creates RangeRef from RangeInstance-like object."""
        ...
        mock_instance.range_id = 123
        mock_instance.user_id = 42
        mock_instance.status = RangeStatus.READY.value

        ref = RangeRef.from_instance(mock_instance)

        assert ref.range_id == 123
        assert ref.user_id == 42
        assert ref.status == RangeStatus.READY

    def test_from_instance_validates_range_id(self):
        """from_instance validates range_id is positive."""
        from unittest.mock import MagicMock
        ...
        mock_instance = MagicMock()
        mock_instance.range_id = 0  # Invalid
        mock_instance.user_id = 42
        mock_instance.status = RangeStatus.READY.value

        with pytest.raises(ValidationError, match="range_id"):
            RangeRef.from_instance(mock_instance)

    def test_from_instance_validates_user_id(self):
        """from_instance validates user_id is positive."""
        from unittest.mock import MagicMock
        ...
        mock_instance = MagicMock()
        mock_instance.range_id = 123
        mock_instance.user_id = -1  # Invalid
        mock_instance.status = RangeStatus.READY.value

        with pytest.raises(ValidationError, match="user_id"):
            RangeRef.from_instance(mock_instance)

    def test_from_instance_converts_status_string_to_enum(self):
        """from_instance converts status string to RangeStatus enum."""
        from unittest.mock import MagicMock
