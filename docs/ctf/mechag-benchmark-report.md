# Operation NORTHSTORM MechaG Benchmark Report
...
- Recommendation: Keep `Hard` and reduce recovery to one dangling commit plus `git show`, or keep the current `git fsck` / `cat-file` chain and raise it to `Expert`.
