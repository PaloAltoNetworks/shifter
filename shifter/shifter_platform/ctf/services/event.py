...
def _schedule_event_tasks(event: CTFEvent) -> None:
    ...
    - Range spin-up before event
    - Event start notification
    - Event end notification
    - Range cleanup after event

    Args:
        event: The CTFEvent to schedule tasks for.
    """
    # TODO: Implement scheduling when scheduler is set up
    logger.debug("Would schedule tasks for event %s", event.id)
...
def _reschedule_event_tasks(event: CTFEvent) -> None:
    ...
    Args:
        event: The CTFEvent to reschedule tasks for.
    """
    # TODO: Implement rescheduling when scheduler is set up
    logger.debug("Would reschedule tasks for event %s", event.id)
