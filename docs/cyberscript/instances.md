...
|-------|------|----------|---------|-------------|
| **`name`** | string | yes | -- | Display name (for example `Attacker`, `Domain Controller`). Must be unique within the scenario. |
...
|-------|------|----------|-------------|
| **`domain_name`** | string | yes | Fully qualified domain name (for example `internal.shifter`). |
| **`netbios_name`** | string | yes | NetBIOS name (for example `INTSHIFTER`). |
