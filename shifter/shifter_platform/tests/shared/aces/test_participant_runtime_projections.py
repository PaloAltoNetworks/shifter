"""Tests for the ACES participant-runtime read-projection seam (#1288).

Drives real ``AcesParticipantRuntimeRecord`` rows through
...
as ``tests/shared/aces/test_projections.py`` for operation records.
"""
...
class TestListParticipantRuntimeRecords:
    def test_redacts_persisted_but_non_response_keys(self):
        ...
        [proj] = list_participant_runtime_records(
            request_id, AcesParticipantRuntimeRecord.RecordKind.PARTICIPANT_RUNTIME
        )
    ...
    def test_response_allowlist_is_a_subset_of_persisted_allowlist(self):
        from shared.schemas.aces_participant_runtime import PAYLOAD_KEYS_BY_RECORD_KIND
    ...
    def test_limit_is_clamped_to_max(self):
        ...
        results = list_participant_runtime_records(
            request_id, AcesParticipantRuntimeRecord.RecordKind.PARTICIPANT_RUNTIME, limit=10_000
        )
    ...
    def test_filters_by_participant_ref(self):
        ...
        results = list_participant_runtime_records(
            request_id,
            AcesParticipantRuntimeRecord.RecordKind.PARTICIPANT_RUNTIME,
            participant_ref="ctf-participant-2",
        )
        assert len(results) == 1
    ...
    def test_empty_when_no_rows(self):
        assert (
            list_participant_runtime_records(uuid4(), AcesParticipantRuntimeRecord.RecordKind.PARTICIPANT_RUNTIME) == []
        )
