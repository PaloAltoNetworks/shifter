"""Project RAES operation status onto the existing range event path (#1274).

This is the engine-side orchestration for the RAES operation-status projection
bridge. It composes the #1273 operation sidecar (durable, idempotent record of
each observation) with the runtime-safe :mod:`shared.raes.status` adapter and
...
1. Read the latest previously recorded operation-status observation for the
   operation (the staleness anchor), via the ``shared.raes.operations`` seam.
...
from shared.raes.status import (
    ProjectionDecision,
    RaesOperationStatusObservation,
...
def project_raes_operation_status(observation: RaesOperationStatusObservation) -> RaesStatusProjection:
    """Persist an RAES operation-status observation and project it to the range path.

    Args:
        observation: The RAES operation-status observation (correlation, contract,
            state, timestamps, and untrusted status reason / diagnostic refs).

    Returns:
        The :class:`RaesStatusProjection` decision. The observation is always
...
def _maybe_enqueue(
    projection: RaesStatusProjection,
    *,
    ...
    if range_obj is None:
        logger.warning(
            "RAES status projection skipped: no range for request_id=%s new_status=%s",
            request_id,
            new_status,
        )
        return
    ...
    if range_obj.status == new_status:
        logger.debug(
            "RAES status projection no-op: range_id=%s already status=%s",
            range_obj.id,
            new_status,
        )
        return
    ...
    logger.info(
        "RAES status projection enqueued: range_id=%s new_status=%s event_id=%s",
        range_obj.id,
        new_status,
        event["event_id"],
    )
