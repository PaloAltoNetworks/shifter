...
Update `shifter/engine/provisioner/catalog/instances.py`:
...
**Files to modify:**
- `shifter/engine/provisioner/catalog/instances.py` - Add secops instance type
- `shifter/engine/provisioner/components/range_stack.py` - Handle secops role in provisioning
- `shifter/engine/provisioner/config.py` - Add secops config fields

### Phase 2: User Data Template

**Create `shifter/engine/provisioner/templates/secops_linux.sh.j2`:**
...
class XSIAMConfig(models.Model):
    ...
    tenant_url = models.URLField()  # for example https://api-tenant.xdr.us.paloaltonetworks.com
...
**Create `shifter/engine/provisioner/plans/secops_setup.py`:**
...
class XSIAMConfig(models.Model):
    ...
    tenant_url = models.URLField(
        help_text="XSIAM API URL (for example https://api-tenant.xdr.us.paloaltonetworks.com)"
    )
    api_key_id = models.CharField(max_length=32)
