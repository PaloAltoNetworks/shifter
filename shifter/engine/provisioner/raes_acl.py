"""Provisioner-side extraction of RAES node ACLs (ADR-031, ADR-032).
...
:class:`~raes_plan_types.RaesPlanAcl` value objects, normalizing action/protocol/
direction to the reference-backend vocabulary. Pure stdlib (no ``raes_*``); any
invalid field raises :class:`~raes_plan_types.RaesPlanError` so a malformed ACL is
...
from raes_plan_types import RaesPlanAcl, RaesPlanError

__all__ = ["build_node_acls"]

#: ACL vocabulary, mirrored from raes_backend_libvirt.acls (the reference backend).
...
def _acl_action(raw: Mapping[str, Any]) -> str:
    ...
    if not token:
        raise RaesPlanError("ACL missing 'action'")
    if token not in _ACL_ACTIONS:
        raise RaesPlanError(f"ACL unknown action {token!r}")
...
def _acl_protocol(raw: Mapping[str, Any]) -> str:
    ...
    if token in _ACL_WILDCARD_PROTOCOLS:
        return "all"
    raise RaesPlanError(f"ACL unknown protocol {token!r}")
...
def _acl_direction(raw: Mapping[str, Any]) -> str:
    ...
    if token not in _ACL_DIRECTIONS:
        raise RaesPlanError(f"ACL unknown direction {token!r}")
    return token
...
def _acl_ports(raw: Mapping[str, Any]) -> tuple[int, ...]:
    ...
    if not isinstance(raw_ports, list | tuple):
        raise RaesPlanError("ACL 'ports' is not a list")
    ports: list[int] = []
    for port in raw_ports:
        if not isinstance(port, int) or isinstance(port, bool) or not 0 < port <= 65535:
            raise RaesPlanError(f"ACL invalid port {port!r}")
        ports.append(port)
    return tuple(ports)
...
def _acl(raw: object, index: int) -> RaesPlanAcl:
    """Build one RaesPlanAcl, failing loud (fail-closed) on any invalid field."""
    if not isinstance(raw, Mapping):
        raise RaesPlanError(f"ACL #{index} is not an object")
    protocol = _acl_protocol(raw)
    ports = _acl_ports(raw)
    if ports and protocol == "all":
        raise RaesPlanError(f"ACL #{index} ports require protocol 'tcp' or 'udp'")
    return RaesPlanAcl(
...
def build_node_acls(raw_acls: object) -> tuple[RaesPlanAcl, ...]:
