"""Shared-native validation contract for RAES package-source catalog records.

RAES package-source rows are *provenance-only*. They reference an authored RAES
...
They MUST NOT carry raw RAES SDL, imported module bodies, generated content,
...
class RaesPackageSourceError(ValueError):
...
def _require_single_line_ref(name: str, value: object, *, required: bool) -> None:
    ...
    if not isinstance(value, str):
        raise RaesPackageSourceError(f"{name} must be a string")
    if not value.strip():
        if required:
            raise RaesPackageSourceError(f"{name} is required")
        return
    if len(value) > _MAX_REF_LEN:
        raise RaesPackageSourceError(f"{name} exceeds {_MAX_REF_LEN} characters")
    if any(ch in value for ch in _FORBIDDEN_CHARS):
        raise RaesPackageSourceError(f"{name} must be a single-line reference, not embedded content")
...
def _require_digest(name: str, value: object, *, required: bool) -> None:
    ...
    if not _DIGEST_RE.match(value):
        raise RaesPackageSourceError(f"{name} must be a 'sha256:<64 hex>' digest")
...
def _validate_scalar(key: str, value: object) -> None:
    ...
    if isinstance(value, str):
        if len(value) > _MAX_PROVENANCE_VALUE_LEN:
            raise RaesPackageSourceError(f"provenance '{key}' value exceeds {_MAX_PROVENANCE_VALUE_LEN} characters")
        if any(ch in value for ch in _FORBIDDEN_CHARS):
            raise RaesPackageSourceError(f"provenance '{key}' value must be single-line, not embedded content")
        return
    raise RaesPackageSourceError(f"provenance '{key}' value must be a scalar or list of scalars")
...
def _require_provenance_size(provenance: dict[str, object]) -> None:
    ...
    try:
        ...
        raise RaesPackageSourceError("provenance must be JSON-serializable") from exc
    if len(encoded.encode("utf-8")) > _MAX_PROVENANCE_BYTES:
        raise RaesPackageSourceError(f"provenance exceeds {_MAX_PROVENANCE_BYTES} bytes")
...
def _require_provenance_entry(key: object, value: object) -> None:
    ...
    if not isinstance(key, str):
        raise RaesPackageSourceError("provenance keys must be strings")
    if key not in PROVENANCE_KEYS:
        raise RaesPackageSourceError(f"provenance key '{key}' is not an allowed reference key")
    if isinstance(value, list):
        if len(value) > _MAX_PROVENANCE_LIST_LEN:
            raise RaesPackageSourceError(f"provenance '{key}' list exceeds {_MAX_PROVENANCE_LIST_LEN} items")
        for item in value:
...
def _validate_provenance(provenance: object) -> Provenance:
    ...
    if not isinstance(provenance, dict):
        raise RaesPackageSourceError("provenance must be a JSON object")
...
def validate_package_source(record: PackageSourceRecord) -> Provenance:
    ...
    Raises:
        RaesPackageSourceError: on any violation.

    Returns:
        The validated provenance dict (``{}`` when ``record.provenance`` is ``None``).
    """
    if record.source_kind not in SOURCE_KINDS:
        raise RaesPackageSourceError(f"source_kind must be one of {sorted(SOURCE_KINDS)}")
    if record.contract_kind not in CONTRACT_KINDS:
        raise RaesPackageSourceError(f"contract_kind must be one of {sorted(CONTRACT_KINDS)}")
    if record.conformance_status not in CONFORMANCE_STATUSES:
        raise RaesPackageSourceError(f"conformance_status must be one of {sorted(CONFORMANCE_STATUSES)}")
