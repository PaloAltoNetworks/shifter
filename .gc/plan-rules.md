...
architectural defaults previously in `AGENTS.md` prose.
...
- Changes to guardrail files (`.github/workflows/**`, `.github/CODEOWNERS`,
  ...
  `.gitleaks.toml`, `.claude/settings.json`, `.claude/hooks/**`,
  `scripts/adr_guard/**`, `docs/adr/**`) MUST stay documented in the
  ADR enforcement docs or registry.
