"""Runtime-safe RAES operation-status -> Shifter ``ResourceStatus`` adapter (#1274).

This module is the single place that knows the RAES operation-status vocabulary
and how it maps onto Shifter's range lifecycle. It is deliberately:

- **Runtime-safe** -- it must not import the RAES SDL (``raes_contracts`` /
  ``raes_backend_protocols``). Only ``shared/raes/manifest.py`` and
  ``shared/raes/runtime_target.py`` import the SDL (a runtime dependency since
  #1262). The known operation-state values are mirrored here as plain strings
  and a drift guard test (``tests/shared/raes/test_status_adapter.py``)
...
- **Pure** -- no database access, no engine/cms/provisioner imports. It maps a
  ...
  orchestration (``engine.services.project_raes_operation_status``) owns
  persistence and outbox enqueue.

The RAES ``OperationStatus`` carries a *coarse operation* lifecycle
...
from shared.schemas.raes_operation import DIAGNOSTIC_REF_KEYS

# Runtime-safe mirror of ``raes_contracts.runtime_state.OperationState``. The SDL
...
RAES_STATE_ACCEPTED = "accepted"
RAES_STATE_RUNNING = "running"
RAES_STATE_SUCCEEDED = "succeeded"
RAES_STATE_FAILED = "failed"
RAES_STATE_CANCELLED = "cancelled"

RAES_OPERATION_STATES: frozenset[str] = frozenset(
    {
        RAES_STATE_ACCEPTED,
        RAES_STATE_RUNNING,
...
class ProjectionDecision(StrEnum):
    """Outcome of projecting one RAES operation-status observation."""
    ...
    STALE = "stale"
    #: Unknown RAES state or unknown intent; never touch range status.
    UNMAPPABLE = "unmappable"


@dataclass(frozen=True)
class RaesStatusProjection:
    """Result of projecting one RAES operation-status observation.
...
@dataclass(frozen=True)
class RaesOperationStatusObservation:
    """One RAES operation-status observation for a Shifter-backed range operation.

    Bundles the correlation, contract, and observation fields so callers pass a
    single value to :func:`engine.services.project_raes_operation_status`.
...
# Explicit (intent x RAES state) -> Shifter ResourceStatus table. This is the
# only mapping authority; adding an RAES status vocabulary or intent means adding
...
# is the safe, operator-visible, reconciler-recoverable outcome. Only RAES-backed
# ranges pass through this adapter, so this does not affect non-RAES range UX.
_STATE_MAP: dict[RangeOperation, dict[str, ResourceStatus]] = {
    RangeOperation.PROVISION: {
        RAES_STATE_ACCEPTED: ResourceStatus.PENDING,
        RAES_STATE_RUNNING: ResourceStatus.PROVISIONING,
        RAES_STATE_SUCCEEDED: ResourceStatus.READY,
        RAES_STATE_FAILED: ResourceStatus.FAILED,
        RAES_STATE_CANCELLED: ResourceStatus.FAILED,
    },
    RangeOperation.DESTROY: {
        RAES_STATE_ACCEPTED: ResourceStatus.DESTROYING,
        RAES_STATE_RUNNING: ResourceStatus.DESTROYING,
        RAES_STATE_SUCCEEDED: ResourceStatus.DESTROYED,
        RAES_STATE_FAILED: ResourceStatus.FAILED,
        RAES_STATE_CANCELLED: ResourceStatus.FAILED,
    },
    RangeOperation.PAUSE: {
        RAES_STATE_ACCEPTED: ResourceStatus.PAUSING,
        RAES_STATE_RUNNING: ResourceStatus.PAUSING,
        RAES_STATE_SUCCEEDED: ResourceStatus.PAUSED,
        RAES_STATE_FAILED: ResourceStatus.FAILED,
        RAES_STATE_CANCELLED: ResourceStatus.FAILED,
    },
    RangeOperation.RESUME: {
        RAES_STATE_ACCEPTED: ResourceStatus.RESUMING,
        RAES_STATE_RUNNING: ResourceStatus.RESUMING,
        RAES_STATE_SUCCEEDED: ResourceStatus.READY,
        RAES_STATE_FAILED: ResourceStatus.FAILED,
        RAES_STATE_CANCELLED: ResourceStatus.FAILED,
    },
...
def _unmappable_reason(
    ...
    if operation_state not in RAES_OPERATION_STATES:
        return f"unknown RAES operation state: {operation_state!r}"
    return None
...
def project_operation_status(
    *,
    ...
    status_reason: str | None = None,
    diagnostic_refs: dict[str, Any] | None = None,
) -> RaesStatusProjection:
    """Project one validated RAES operation-status observation to a range status.

    Args:
        operation_state: RAES ``OperationState`` value (must be a known state).
    ...
    Returns:
        An :class:`RaesStatusProjection`. ``APPLY`` carries the target
