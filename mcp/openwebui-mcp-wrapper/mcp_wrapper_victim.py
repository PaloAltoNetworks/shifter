"""
title: Shifter Victim MCP Tools
author: Shifter Team
author_url: https://github.com/paloaltonetworks/shifter
description: Execute MCP tools on your Shifter Victim range. Enables AI-driven scenario setup through the Shifter cyber range platform.
...
class Tools:
    """
    OpenWebUI Tool wrapper for Shifter Victim MCP integration.

    Forwards the user's OAuth token to mcp-shifter-victim to enable per-user
    MCP sessions scoped to their active Victim range.
    ...
    CLIENT_NAME = "shifter-openwebui-wrapper-victim"
    ...
    def __init__(self):
        ...
        self._session_token: Optional[str] = (
            None  # Track which token created the session
        )

    class Valves(BaseModel):
        """Admin-configurable settings for the Victim MCP wrapper."""

        mcp_server_url: str = Field(
            default="http://mcp-shifter-victim:3002",
            description="URL of the mcp-shifter-victim server",
        )
        request_timeout: int = Field(
            default=30, description="Timeout in seconds for MCP requests"
        )
    ...
    async def _send_initialized_notification(
        self, oauth_token: Optional[dict], client
    ) -> None:
        """
        ...
        await client.post(
            f"{self.valves.mcp_server_url}/mcp", headers=headers, json=payload
        )

    async def _ensure_initialized(
        self, oauth_token: Optional[dict], __event_emitter__=None
    ) -> None:
        """
            ...
            await __event_emitter__(
                {
                    "type": "status",
                    "data": {
                        "description": "Connecting to your Victim range...",
                        "done": False,
                    },
                }
            )
                ...
                await __event_emitter__(
                    {
                        "type": "status",
                        "data": {
                            "description": "Connected to Victim range",
                            "done": True,
                        },
                    }
                )
    ...
    async def list_mcp_tools(
        self, __oauth_token__: Optional[dict] = None, __event_emitter__=None
    ) -> str:
        """
        List available MCP tools for your Victim range.

        Returns a list of tools that can be used to interact with your
        Shifter Victim instance, including their names and descriptions.
        """
        try:
    ...
    async def run_mcp_tool(
        ...
        """
        Execute an MCP tool on your Victim range.

        Runs the specified tool with the given arguments on your Shifter
        Victim instance. Use list_mcp_tools first to see available tools.

        :param tool_name: Name of the MCP tool to execute (e.g., "victim_run_command", "victim_info")
