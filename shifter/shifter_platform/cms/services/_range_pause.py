...
def _engine_pause_range_call(request_id: Any) -> Any:  # NOSONAR (late-bind proxy)
...
def pause_range(user: User, range_id: int) -> None:
    ...
    try:
        ...
        if not _engine_pause_range_call(request_id):
...
def pause_range_by_request_id(user: User, request_id: str) -> None:
    ...
    try:
        ...
        if not _engine_pause_range_call(instance.request.request_id):
            instance.status = ResourceStatus.READY.value
            instance.save(update_fields=["status"])
            logger.warning(
