...
from engine.provisioner.executors.base import CommandResult
        ...
        try:
            ...
            while True:
                ...
                if channel.eof_received:
                    ...
                    while True:
                        try:
                            drain_data = channel.recv(4096)
                            if not drain_data:  # Empty bytes = channel closed, no more data
                                break
                            chunk_count += 1
                            logger.info(f"Drain chunk {chunk_count}: {len(drain_data)} bytes")
                            output += drain_data.decode("utf-8", errors="replace")
