"""IPv4-only network address-family admission for the ACES provisioning backend (#1568).
...
compiled ACES ``network`` resource's CIDR against it so
:mod:`shared.aces.runtime_target` can reject a non-IPv4 network as an unsupported
