"""Tests for the ACES participant-runtime Mission Control presentation projection (#1290).

Drives real ``AcesParticipantRuntimeRecord`` rows through
``shared.aces.presentation.build_range_participant_runtime_projection`` and
asserts: ``None`` for ranges with zero participant-runtime rows (non-ACES
...
backend-command channel) -- mirrors ``tests/shared/aces/test_presentation.py``
for the existing #1276 ``aces_projection`` slice.
"""
...
class TestBuildRangeParticipantRuntimeProjection:
    ...
    def test_none_even_with_instances_and_no_rows(self):
        """Non-ACES ranges with real instances stay boring (projection absent)."""
