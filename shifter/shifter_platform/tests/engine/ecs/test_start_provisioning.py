"""Behavior tests for start_provisioning().

start_provisioning() delegates to _start_ecs_task with the "provision" command.
Driven through the real dispatch with the ECS client mocked at the ``boto3``
boundary; the delegation is asserted by the command line reaching ``boto3``.
"""
...
class TestStartProvisioning:
    def test_dispatches_provision_command(self, aws_ecs_configured, ecs_client):
        ...
        assert run_task_command(ecs_client) == ["range", "provision", "--range-id", "42", "--user-id", "7"]

    def test_returns_task_arn_on_success(self, aws_ecs_configured, ecs_client):
        from engine.ecs import start_provisioning

        assert start_provisioning(range_id=42, user_id=7) == TASK_ARN

    def test_returns_none_when_ecs_not_configured(self, aws_ecs_unconfigured, ecs_client):
        from engine.ecs import start_provisioning

        assert start_provisioning(range_id=42, user_id=7) is None
