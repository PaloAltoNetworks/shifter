...
- Identity Platform client-side configuration no longer contains the `allowedEmails` external-user whitelist. It is enforced only server-side by `IdentityPlatformBackend.is_allowed_identity_email` during Django session exchange.
...
- Identity Platform blocking hooks are disabled in this org because the required public unauthenticated invocation conflicts with org policy. The shipped corporate gate is Django session exchange (`config/identity_platform.py::_assert_account_can_create_app_session` plus `IdentityPlatformBackend.is_allowed_identity_email`).
- GCP range image lookup now mirrors the AWS SSM-backed contract. `shifter/engine/provisioner/main.py::get_gdc_image_url` reads the image URL from Secret Manager (`shifter-<environment>-range-image-<type>`) at range-create time with a module-level cache, alongside the existing `get_ami_id` for the AWS side. `GDCVMRuntimeProfile` and `GDCPaloAltoVMSeriesConfig` no longer carry image URLs (sizing only); `_load_gdc_vm_profile` and `load_gdc_palo_alto_vmseries_config` no longer read `GDC_*_IMAGE_URL` / `GDC_VMSERIES_IMAGE_URL` env vars; the overlay `platform-runtime.env` drops those placeholders. Rotating a range image no longer requires a portal redeploy.
...
- Client-side QR code rendering for TOTP MFA enrollment. The Identity Platform login page now shows a scannable QR generated in the browser from `TotpSecret.generateQrCodeUrl()` via the `node-qrcode` ES module bundled by jsdelivr, with the raw secret collapsed behind a "Can't scan?" disclosure as a manual-entry fallback. Replaces the previous UX that dumped the raw `otpauth://` URI and base32 secret as unusable text blocks.
- `google_secret_manager_secret.range_image` declares five empty Secret Manager secrets (`range-image-kali`, `-ubuntu`, `-windows`, `-dc`, `-vmseries`) in `platform-core`. Values are populated out-of-band by the Packer build/promote pipeline; the provisioner reads the latest version on every range create.
...
- Cloud Armor now bypasses the narrow authenticated POST surfaces that were false-positiving on CRS SQLi rule `942200` before they reached Django: Identity Platform session exchange, Mission Control upload/range APIs, Scenario Editor, Experiments, NGFW API, Credentials API, Guacamole API, and Mission Control files POSTs. Those paths remain protected by Django session auth, CSRF, strict request parsing, and subsystem validation, and AWS is unchanged.
