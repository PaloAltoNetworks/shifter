# EC2 Module - AgentChat (OpenWebUI + Bedrock Access Gateway)
#
# Creates:
# - EC2 instance with Docker (Amazon Linux 2023)
# - Security group (egress only, no ingress - SSM tunnel access)
# - IAM role and instance profile (Bedrock invoke, SSM)
# - Secrets Manager secret for BAG API key
...
resource "aws_secretsmanager_secret_version" "bag_api_key" {
  secret_id     = aws_secretsmanager_secret.bag_api_key.id
...
resource "aws_security_group" "this" {
  ...
  description = "Security group for AgentChat EC2 instance"
  vpc_id      = var.vpc_id
...
resource "aws_security_group_rule" "egress_all" {
  ...
  description       = "Allow all outbound (Bedrock API, GHCR, GitHub)"
}
