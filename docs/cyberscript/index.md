...
Templates are YAML files. The filename stem is the `id` field (for example `basic.yaml` has `id: basic`). Loaded via `yaml.safe_load` and validated against `ScenarioTemplate`.
