"""Persistence helpers for RAES participant-runtime sidecar records (#1288).

Mirrors :mod:`shared.raes.operations` (the incumbent
:class:`shared.models.RaesOperationRecord` write seam from #1273/#1274).
"""
...
class RaesParticipantRuntimeRecordConflict(ValueError):
...
def _reconcile_payload_identity(payload: dict[str, Any], **identity: object) -> None:
    ...
    for key, expected in identity.items():
        ...
        if actual is not None and actual != expected:
            raise RaesParticipantRuntimeRecordConflict(
...
def _payload_identity(payload: dict[str, Any], key: str) -> str:
    ...
    if not isinstance(value, str) or not value.strip():
        raise RaesParticipantRuntimeRecordError(f"payload {key!r} is required to identify the record")
    return value
...
class ParticipantRecordWriteOptions:
    ...
    owner: str = RaesParticipantRuntimeRecord.Owner.SHARED
    range_id: UUID | str | None = None
    range_instance_id: UUID | str | None = None


@dataclass(frozen=True)
class RaesParticipantRuntimeRecordWrite:
    """Input contract for writing one RAES participant-runtime sidecar record."""
    ...
    range_id: UUID | str | None = None
    range_instance_id: UUID | str | None = None
    contract_kind: str = RaesParticipantRuntimeRecord.ContractKind.RAES
    contract_profile: str = SHIFTER_BACKEND_PROFILE
    participant_runtime_profile: str = DEFAULT_PARTICIPANT_RUNTIME_PROFILE
    owner: str = RaesParticipantRuntimeRecord.Owner.SHARED
    retention_class: str = RaesParticipantRuntimeRecord.RetentionClass.DEFAULT
    redaction_state: str = RaesParticipantRuntimeRecord.RedactionState.SANITIZED
    retention_expires_at: datetime | None = None


def _idempotency_lookup(write: RaesParticipantRuntimeRecordWrite) -> dict[str, Any]:
...
def _assert_replay_matches(
    existing: RaesParticipantRuntimeRecord,
    *,
    write: RaesParticipantRuntimeRecordWrite,
    payload_digest: str,
) -> None:
...
def persist_raes_participant_runtime_record(
    write: RaesParticipantRuntimeRecordWrite,
) -> RaesParticipantRuntimeRecord:
    """Create or return an idempotent RAES participant-runtime sidecar record.
    ...
    try:
        with transaction.atomic():
            record, created = RaesParticipantRuntimeRecord.objects.get_or_create(**lookup, defaults=defaults)
    except IntegrityError:
        record = RaesParticipantRuntimeRecord.objects.get(**lookup)
        created = False
...
def persist_participant_implementation_record(
    *,
    ...
    payload: dict[str, Any],
    diagnostic_refs: dict[str, Any] | None = None,
    owner: str = RaesParticipantRuntimeRecord.Owner.PROVISIONER,
) -> RaesParticipantRuntimeRecord:
    ...
    not touch the ``RaesParticipantRuntimeRecord`` model. The idempotency key
    ...
    non-default version use ``persist_raes_participant_runtime_record`` directly.
    """
    ...
    return persist_raes_participant_runtime_record(write)
...
def persist_participant_runtime_record(
    *,
    ...
    payload: dict[str, Any],
    diagnostic_refs: dict[str, Any] | None = None,
    owner: str = RaesParticipantRuntimeRecord.Owner.ENGINE,
) -> RaesParticipantRuntimeRecord:
    ...
    version use ``persist_raes_participant_runtime_record`` directly.
    """
...
def persist_participant_behavior_history_record(
    *,
    ...
    ``persist_raes_participant_runtime_record`` directly.
    """
    ...
    write = RaesParticipantRuntimeRecordWrite(
        ...
        record_kind=RaesParticipantRuntimeRecord.RecordKind.PARTICIPANT_BEHAVIOR_HISTORY,
...
def persist_participant_evidence_record(
    *,
    ...
    write = RaesParticipantRuntimeRecordWrite(
        ...
        record_kind=RaesParticipantRuntimeRecord.RecordKind.PARTICIPANT_EVIDENCE,
